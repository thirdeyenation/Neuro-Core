...
def install() -> bool:
    ...
    # --- Startup decoration: re-apply @extensible at FULL identity -------
    # Re-applies the framework's public decorator to the three Memory
    # methods so the _functions handler tree fires for host-initiated
    # operations. Idempotent and exception-safe (ARC conditions 3/4).
    try:
        from usr.plugins.neuro_core.helpers.decorate import decorate_memory
        decorate_memory()
    except Exception as e:
        print(f"[neuro_core] startup decoration non-fatal: {e}")
